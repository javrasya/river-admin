...
from river_admin.views import get, ApprovalHookDto
...
def list_approval_hooks(request, transition_approval_id):
    ...
    return Response(ApprovalHookDto(transition_approval.on_approved_hooks.filter(object_id__isnull=True), many=True).data, status=HTTP_200_OK)
